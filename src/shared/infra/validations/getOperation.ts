...
const schema = Zod.object({
  page: Zod.string().min(1).transform(v => Number(v)),
  size: Zod.string().min(1).transform(v => Number(v)),
  criteria: Zod.string().optional(),
  column: Zod.enum(['cost', 'type']).default('type'),
  direction: Zod.enum(PaginationOrder).default('ASC'),
});

export const validateGetOperation = (body: Record<string, unknown>): PaginationProps => {
